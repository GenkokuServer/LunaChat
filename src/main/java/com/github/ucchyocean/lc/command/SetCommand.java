...
public class SetCommand extends SubCommandAbst {
    ...
    public boolean runCommand(CommandSender sender, String label, String[] args) {
        ...
        if (args.length >= 3 && args[1].equalsIgnoreCase("default")) {
            ...
            if (args.length >= 4) targetChannel = api.getChannel(args[3]);
            else if (sender instanceof Player) targetChannel = api.getDefaultChannel(sender.getName());
            ...
            if (target != null) sendResourceMessage(target, PREINFO, "cmdmsgSet", targetChannel.getName());
