...
public class LunaChatCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if (LunaChat.getInstance().getLunaChatConfig().isDisableChannelChat() && !sender.isOp()) {
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        if (args.length == 1) {
        ...
        } else if (args.length == 2 && (args[0].equalsIgnoreCase("join") || args[0].equalsIgnoreCase("info"))) {
            // 参加可能チャンネル名で補完する
            ...
            ArrayList<String> items = new ArrayList<>();
            for (String name : getListCanJoin(sender))
                if (name.toLowerCase().startsWith(args[1].toLowerCase())) items.add(name);
        ...
        } else if (args.length == 2 && (args[0].equalsIgnoreCase("dic") || args[0].equalsIgnoreCase("dictionary"))) {
            ...
            return items;
                ...
                (args[0].equalsIgnoreCase("dic") || args[0].equalsIgnoreCase("dictionary")) && args[1].equalsIgnoreCase("remove")) {
            // 辞書に登録されているワードで補完する
            ArrayList<String> items = new ArrayList<>();
     ...
     */
    private void sendResourceMessage(CommandSender sender, String key, Object... args) {
        String org = Resources.get(key);
        if (org == null || org.equals("")) return;
        sender.sendMessage(String.format(LunaChatCommand.PREERR + org, args));
    }
    ...
    private ArrayList<String> getListCanJoin(CommandSender sender) {
        ...
        for (Channel channel : LunaChat.getInstance().getLunaChatAPI().getChannels()) {
            ...
            if (sender instanceof Player && !channel.getMembers().contains(cp) && !channel.isGlobalChannel() && !channel.isVisible())
                continue;
    ...
    private ArrayList<String> getListCanRemove(CommandSender sender) {
        ...
        for (Channel channel : LunaChat.getInstance().getLunaChatAPI().getChannels()) {
            ...
            // グローバルチャンネルは対象外
            if (channel.isGlobalChannel()) continue;
