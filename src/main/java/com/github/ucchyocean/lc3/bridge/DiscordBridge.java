...
public class DiscordBridge extends ListenerAdapter {
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		if (event.getMember() == null || jda == null || event.getAuthor().equals(jda.getSelfUser())) {
			return;
		}
		...
		boolean isRelayDefaultChannel = channels.stream()
				.anyMatch(channel -> channel.getDiscordChannelId().equals(event.getChannel().getId()) && channel.isRelayDiscordChatDefaultChannel());

		if (isRelayDefaultChannel){
			Channel discordDefaultChannel = api.getChannel(config.getDiscordDefaultSendLunaChatChannel());
			if (discordDefaultChannel != null){
				discordDefaultChannel.chatFromOtherSource(event.getMember().getEffectiveName(),null, message.toString());
			}
			return;
		}
		...
		channels.stream()
				...
				.forEach(channel -> channel.chatFromOtherSource(event.getMember().getEffectiveName(),null, message.toString()));
	}
